...
<nav id="sidebar" class="ego-sidebar" aria-label="Main sidebar">
    ...
    <div class="ego-sidebar__scroll">
        <ul class="ego-nav" role="list">
            ...
            {{-- DASHBOARD --}}
            <li class="ego-item" data-title="Trang chủ">
...
<li class="ego-item" data-title="Booking phòng họp">
...
{{-- EGO_BOOKING_ROOM_MENU_END --}}
            ...
            @php
                ...
                $canCustomerMenu =
                    $egoCustomerMenuIsKho
                    || auth()->user()->can('customer.view_all')
                    || auth()->user()->can('customer.view_own')
                    || auth()->user()->can('customer.view_sales_all')
                    || auth()->user()->can('customer.create')
                    || auth()->user()->can('customer.update_all');
            @endphp

            @if($canCustomerMenu)
                <li class="ego-item ego-item--has-sub" data-title="Khách hàng" data-ego-sub="true">
            ...
            {{-- ĐƠN HÀNG --}}
            <li class="ego-item ego-item--has-sub" data-title="Đơn hàng" data-ego-sub="true">
                <a href="#menuOrders"
                   class="ego-link {{ request()->routeIs('orders.*') || request()->routeIs('payment-methods.*') || request()->is('companies*') || request()->routeIs('serial-warranty.*') ? 'active' : '' }}"
                   data-bs-toggle="collapse"
                   data-ego-type="toggle"
                   aria-expanded="{{ request()->routeIs('orders.*') || request()->routeIs('payment-methods.*') || request()->is('companies*') || request()->routeIs('serial-warranty.*') ? 'true' : 'false' }}"
                ...
                <ul id="menuOrders"
                    class="ego-sub collapse {{ (request()->routeIs('orders.*') || request()->routeIs('payment-methods.*') || request()->is('companies*') || request()->routeIs('serial-warranty.*')) ? 'show' : '' }}"
                    data-ego-submenu>
                    <li>
                    ...
                    <li>
                        <a href="{{ route('payment-methods.index') }}"
                           class="ego-sublink {{ active_route('payment-methods.*') }}"
                           data-ego-type="nav">
            ...
            {{-- CÔNG TRÌNH --}}
            @hasanyrole('ky_thuat|accounting|warehouse|kho|admin|sales')
                <li class="ego-item ego-item--has-sub" data-title="Công trình" data-ego-sub="true">
                    ...
                    <ul id="menuConstruction"
                        ...
                        <li>
                            <a href="{{ url('/cong-trinh') }}"
                               class="ego-sublink {{ request()->is('cong-trinh*') ? 'active' : '' }}"
                               data-ego-type="nav">
                                Công trình
                            </a>
                        </li>
            ...
            {{-- ĐỀ NGHỊ THANH TOÁN --}}
            <li class="ego-item" data-title="Đề nghị thanh toán">
...
@endauth
{{-- KỸ THUẬT --}}
@hasanyrole('ky_thuat|accounting|admin')
<li class="ego-item ego-item--has-sub" data-title="Kỹ thuật" data-ego-sub="true">
    <a href="#menuKyThuat"
...
@endhasanyrole
            {{-- SALES --}}
            @hasanyrole('sales|sales_manager|accounting|admin')
                <li class="ego-item ego-item--has-sub" data-title="Sales" data-ego-sub="true">
                    <a href="#menuSales"
...
@else
    ...
    @endif
            ...
            {{-- MARKETING --}}
            @hasanyrole('marketing|marketing_manager|admin|accounting')
                <li class="ego-item ego-item--has-sub ego-item--modern" data-title="Marketing" data-ego-sub="true">
            ...
            {{-- CÔNG VIỆC --}}
            <li class="ego-item ego-item--has-sub" data-title="Công việc" data-ego-sub="true">
                <a href="#menuTasks"
            ...
            @php
                ...
                $canProductMenu = $u && (
                    $u->hasAnyRole(['admin','warehouse']) ||
                    $u->can('product.view') ||
                    $u->can('products.manage') ||
                    $u->can('categories.manage') ||
                    $u->can('brands.manage') ||
                    $u->can('price-tiers.manage') ||
                    $u->can('warehouse.manage') ||
                    $u->can('warehouse.view')
                );

                $canSeeInputProducts = $u && $u->hasAnyRole(['admin','warehouse','accounting']);
            @endphp

          @if($canProductMenu)
    <li class="ego-item ego-item--has-sub" data-title="Sản phẩm" data-ego-sub="true">
        <a href="#menuSP"
...
{{-- TÀI CHÍNH --}}
@hasanyrole('admin|accounting|hr')
<li class="ego-item ego-item--has-sub" data-title="Tài chính" data-ego-sub="true">
    <a href="#menuFinance"
...
@endif
            ...
            {{-- NHÂN SỰ --}}
@hasanyrole('admin|accounting|hr')
<li class="ego-item ego-item--has-sub" data-title="Nhân sự" data-ego-sub="true">
        <a href="#menuNhanSu"
        ...
        <span>Quy trình phân bổ VPP</span>
            ...
            <li class="ego-item ego-item--nested ego-item--has-sub" data-title="Chấm công" data-ego-sub="true">
                ...
                <ul id="menuNhanSuChamCong"
                    ...
                    <li>
                        <a href="{{ route('hr.leave.index', ['request_type' => 'leave']) }}"
                           class="ego-sublink {{ request('request_type') === 'leave' ? 'active' : '' }}"
                           data-ego-type="nav">
                            Đơn xin nghỉ
                        </a>
                    </li>
                    ...
                    <li>
                        <a href="{{ route('hr.leave.index', ['request_type' => 'wfh']) }}"
                           class="ego-sublink {{ request('request_type') === 'wfh' ? 'active' : '' }}"
                           data-ego-type="nav">
                            Đơn xin làm online
                        </a>
                    </li>
...
@endhasanyrole
    ...
    @auth
        {{-- ===== MINI STATUS FOOTER - CLEAN V3 ===== --}}
        @php
            $egoSidebarCompanies = collect();
            $egoCurrentCompanyId = session('ego_company_id')
            ...
            try {
                if (\Illuminate\Support\Facades\Schema::hasTable('companies')) {
                    $egoCompanyQuery = \Illuminate\Support\Facades\DB::table('companies')
                        ->select('id', 'code', 'name');

                    if (\Illuminate\Support\Facades\Schema::hasColumn('companies', 'is_active')) {
                        $egoCompanyQuery->where('is_active', 1);
                    }

                    $egoSidebarCompanies = $egoCompanyQuery->orderBy('id')->get();

                    if (!$egoCurrentCompanyId && $egoSidebarCompanies->count()) {
                        $egoCurrentCompanyId = $egoSidebarCompanies->first()->id;
                    }
                }
            } catch (\Throwable $e) {
                $egoSidebarCompanies = collect();
            ...
            $egoCompanySwitchAction = \Illuminate\Support\Facades\Route::has('company-context.store')
                ? route('company-context.store')
                : url('/chon-cong-ty');

            $egoCompanyLabel = function ($company) {
                $raw = strtoupper(trim((string)($company->code ?? '')));
                $name = mb_strtolower(trim((string)($company->name ?? '')));

                if (str_contains($raw, 'VN') || str_contains($name, 'việt')) return 'VN';
                if (str_contains($raw, 'QT') || str_contains($raw, 'INT') || str_contains($name, 'quốc tế')) return 'QT';

                return $raw !== '' ? mb_substr($raw, 0, 3) : ('CT' . $company->id);
            };
        @endphp

        <div class="ego-sidebar__footer">
            <div class="ego-status-card ego-status-card-v3" aria-label="Trạng thái hôm nay">
                <div class="ego-status-card__glow"></div>

                <div class="ego-status-v3-head">
                    <div>
                    ...
                    <div class="ego-status-v3-head-actions">
                        <span class="ego-status-chip">LIVE</span>
                        <button type="button" class="ego-status-v3-toggle" title="Thu gọn / mở rộng">
                            <i class="bi bi-chevron-down"></i>
                        </button>
                    </div>
                </div>

                @if($egoSidebarCompanies->count())
                    <div class="ego-status-company-v3">
                        <div class="ego-status-company-v3-label">
                            <i class="bi bi-building"></i>
                            <span>Chọn công ty</span>
                        </div>

                        <div class="ego-status-company-v3-switch">
                            @foreach($egoSidebarCompanies as $company)
                                @php
                                    $isActiveCompany = (string)$egoCurrentCompanyId === (string)$company->id;
                                    $labelCompany = $egoCompanyLabel($company);
                                @endphp

                                <form method="POST" action="{{ $egoCompanySwitchAction }}">
                                    @csrf
                                    <input type="hidden" name="company_id" value="{{ $company->id }}">
                                    <button type="submit"
                                            class="ego-status-company-v3-btn {{ $isActiveCompany ? 'active' : '' }}"
                                            title="{{ $company->name ?? $labelCompany }}">
                                        {{ $labelCompany }}
                                    </button>
                                </form>
                            @endforeach
                ...
                <div class="ego-side-lang-box ego-side-lang-box-v3" title="Chuyển ngôn ngữ">
...
<!-- EGO_REMOVE_CHAT_BUBBLE_START -->
<style>
/* Ẩn nút chat nổi góc phải dưới */
#chatWidget,
#chat-widget,
.chat-widget,
.chat-popup,
...
(function () {
    function removeChatBubble() {
        document.querySelectorAll(
            '#chatWidget,#chat-widget,.chat-widget,.chat-popup,.chat-box,.chat-panel,.chat-float,.floating-chat,.chat-launcher,.chat-toggle,.chat-fab,[data-chat-widget],[data-chat-toggle]'
        ).forEach(function (el) {
            el.remove();
        });
        ...
        document.querySelectorAll('button, a, div').forEach(function (el) {
            ...
            var nearRight = rect.right > window.innerWidth - 120;
            var nearBottom = rect.bottom > window.innerHeight - 120;
            var sizeOk = rect.width >= 42 && rect.width <= 90 && rect.height >= 42 && rect.height <= 90;
            var round = parseFloat(st.borderRadius) >= 20;
            var hasBlueBg = (st.backgroundColor || '').includes('rgb');

            if (isFixed && nearRight && nearBottom && sizeOk && round && hasBlueBg) {
                el.remove();
            }
        });
    }

    document.addEventListener('DOMContentLoaded', removeChatBubble);
    removeChatBubble();
    setInterval(removeChatBubble, 500);
})();
</script>
<!-- EGO_REMOVE_CHAT_BUBBLE_END -->
...
<!-- EGO_KILL_LEGACY_COMPANY_SWITCH_START -->
<style>
    ...
    html body #sidebar .ego-side-lang-box,
    html body #sidebar .ego-side-lang-box-v3{
        width:100% !important;
    ...
    html body #sidebar .ego-side-lang-switch{
        display:grid !important;
    ...
    html body #sidebar .ego-side-lang-btn{
    ...
    html body #sidebar .ego-side-lang-btn.active{
...
(function(){
    function allowed(el){
        return !!el.closest('.ego-status-company-v3, .ego-status-company-v3-switch, .ego-side-lang-box, .ego-side-lang-box-v3');
    }

    function killLegacyCompanySwitch(){
        ...
        sidebar.querySelectorAll('div, span, form, button, li').forEach(function(el){
            if(allowed(el)) return;
            if(el.closest('.ego-side-lang-box, .ego-side-lang-box-v3')) return;
            ...
            if(txt === 'VN QT' || txt === 'QT VN' || txt === 'VNQT' || txt === 'QTVN'){
                el.remove();
            }
        });
    }

    document.addEventListener('DOMContentLoaded', killLegacyCompanySwitch);
    setTimeout(killLegacyCompanySwitch, 100);
    setTimeout(killLegacyCompanySwitch, 400);
    setTimeout(killLegacyCompanySwitch, 1000);
    setInterval(killLegacyCompanySwitch, 500);

    if(window.MutationObserver){
        var sidebar = document.querySelector('#sidebar');
        if(sidebar){
            new MutationObserver(killLegacyCompanySwitch).observe(sidebar, {
                childList:true,
                subtree:true,
                characterData:true
            });
...
<!-- EGO_KILL_LEGACY_COMPANY_SWITCH_END -->
