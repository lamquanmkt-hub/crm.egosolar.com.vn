...
## Cập nhật v2.1 — Popup, Serial-first, Sửa chữa độc lập

- **Nhập liệu bằng popup**: mọi thao tác nhập/duyệt/xác nhận (tạo phiếu, chẩn đoán, báo giá, khách xác nhận, giữ/xuất kho, nhận/thay hàng, thu hồi, QA, bàn giao, hoàn tất, hủy, tải file) mở modal ngay trên trang (`x-wx-modal` + `public/js/warranty-modals-v2.js`, gửi AJAX, lỗi hiện trong popup, lưu xong tải lại đúng phiếu/giữ bộ lọc). Trang chính chỉ xem/lọc/timeline; nút hành động nằm ngay trên bước hiện tại của timeline.
- **Đổi hàng bảo hành — chỉ nhập SERIAL**: `SerialLookup` tự truy ngược sản phẩm/model, khách + SĐT, đơn hàng gốc, ngày bán, công trình + địa chỉ, bảo hành, trạng thái/kho, lịch sử đổi/sửa, phiếu đang mở. Server tự lấy site/order/customer từ serial (bỏ qua giá trị client gửi). Serial có phiếu mở ⇒ báo mã phiếu và chặn (service + unique key). Không đủ điều kiện ⇒ nút **Chuyển sang Sửa chữa tính phí** (mang theo serial) hoặc ngoại lệ (có lý do, người khác duyệt).
- **Sửa chữa tính phí độc lập (9 bước)**: khách hàng + thiết bị + lỗi là dữ liệu chính; KHÔNG cần đơn hàng/công trình/serial trong CRM. Khách chọn từ CRM hoặc nhập mới ngay trong popup (SĐT trùng ⇒ gợi ý chọn khách hiện có); thiết bị lưu snapshot trên phiếu (`customer_*`, `device_*`, `received_condition`...). Nếu serial tình cờ có trong CRM: chỉ hiện khối “Thông tin tham chiếu trong hệ thống”. Migration `2026_09_23_000100_warranty_repair_external_intake.php`.
