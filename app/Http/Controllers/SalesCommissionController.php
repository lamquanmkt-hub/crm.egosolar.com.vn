...
use App\Models\User;
use App\Models\SalesDailyKpiPostLink;
...
{
    private const EXCLUDED_SALES_NAME = 'Ngọc Trân';
    ...
    private function defaultSalesKpiSettings(): array
    {
        return [
    ...
    private function salesKpiSettingsPath(): string
    {
        return storage_path('app/sales_kpi_settings.json');
    }

    private function salesKpiSettings(): array
    {
    ...
    private function kpiSettingEnabled(array $settings, string $key): bool
    {
        return (int) ($settings[$key] ?? 0) === 1;
    }

    private function salesKpiExtraMetricDefinitions(): array
    {
        return [
    ...
    private function normalizeExtraMetrics(array $input): array
    {
        $extra = [];
    ...
    private function decodeExtraMetrics($entry): array
    {
    ...
    private function salesKpiTargets(): array
    {
    ...
    private function buildQuery(Request $request, $user, ?Carbon $from, ?Carbon $to)
    {
        ...
        */
        ...
        $q = DB::table('crm_orders as o')
            ...
            ->where('u.name', '!=', self::EXCLUDED_SALES_NAME)
    ...
    public function index(Request $request)
    {
        ...
        */
        ...
        $salesOptions = DB::table('users')
            ...
            ->where('users.name', '!=', self::EXCLUDED_SALES_NAME)
    ...
    public function exportExcel(Request $request)
    {
        ...
        $filterSalesId = (int) $request->get('sales_id', 0);
        ...
        try {
            ...
            $salesUsers = \Illuminate\Support\Facades\DB::table('users')
                ...
                ->where('name', '!=', self::EXCLUDED_SALES_NAME)
                ->orderBy('name')
                ->get();
        }
    ...
    private function calculateDailyKpi(array $payload): array
    {
        ...
        if ($this->kpiSettingEnabled($settings, 'enable_posts')) {
            $checks['posts'] = $postsCount >= (int) $settings['posts_target'];
        }

        if ($this->kpiSettingEnabled($settings, 'enable_calls')) {
            $checks['calls'] = $callsAnswered >= (int) $settings['calls_target'];
        }

        if ($this->kpiSettingEnabled($settings, 'enable_company_data')) {
            $checks['company_data'] = $companyDataCalled >= (int) $settings['company_data_target'];
        }

        if ($this->kpiSettingEnabled($settings, 'enable_follow_up')) {
            $checks['follow_up'] = $followedUpAllPrevious;
        }

        $extraMetrics = $this->normalizeExtraMetrics(is_array($payload['extra_metrics'] ?? null) ? $payload['extra_metrics'] : []);

        foreach ($this->salesKpiExtraMetricDefinitions() as $module) {
            if (!$this->kpiSettingEnabled($settings, $module['enabled_key'])) {
                continue;
            }
        ...
        $penaltyAmount = $this->kpiSettingEnabled($settings, 'enable_penalty')
    ...
    public function kpiDashboard(Request $request)
{
    ...
    */
    ...
    if ($selectedUserId) {
        ...
        $preview = $this->calculateDailyKpi([
            ...
            'extra_metrics' => method_exists($this, 'decodeExtraMetrics') ? $this->decodeExtraMetrics($currentEntry) : [],
        ]);
    } else {
    ...
    */
    ...
    return view('sales.kpi.index', [
        ...
        'dailyRows' => $dailyRows,
        'targets' => $this->salesKpiTargets(),
        'kpiExtraModules' => $this->salesKpiExtraMetricDefinitions(),
    ...
    public function kpiMyForm(Request $request)
{
    ...
    $extraMetrics = $this->decodeExtraMetrics($currentEntry);
   ...
   public function kpiMyStore(Request $request)
{
    ...
    $validated['extra_metrics'] = $this->normalizeExtraMetrics($validated['extra_metrics'] ?? []);
    ...
    public function kpiSettingsSave(Request $request)
    {
        ...
        foreach ($this->defaultSalesKpiSettings() as $key => $default) {
