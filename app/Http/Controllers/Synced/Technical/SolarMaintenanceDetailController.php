...
class SolarMaintenanceDetailController extends Controller
{
    ...
    public function site(Request $request, int $site): View|RedirectResponse
    {
        ...
        return view('synced.technical.maintenance.site-show', [
            'site' => $siteModel,
            'schedules' => $schedules,
            'cycles' => $cycles,
            'documents' => $documents,
    ...
    public function show(Request $request, SolarMaintenanceSchedule $schedule): View|RedirectResponse
    {
        ...
        if (SchemaCache::hasTable('solar_maintenance_work_items')) {
            $relations[] = 'attachments.workItem:id,maintenance_schedule_id,title';
        ...
        if (SchemaCache::hasTable('solar_maintenance_checklist_items')) {
            $relations[] = 'checklistItems.attachments';
        }
        ...
        $workItems = SchemaCache::hasTable('solar_maintenance_work_items')
            ? $schedule->workItems
            : collect();
        $legacyChecklistItems = SchemaCache::hasTable('solar_maintenance_checklist_items')
            ? $schedule->checklistItems
            : collect();
        $comments = SchemaCache::hasTable('solar_maintenance_comments')
            ? $schedule->comments
            : collect();
        $legacyChecklistFallback = $workItems->isEmpty() && $legacyChecklistItems->isNotEmpty();
        $workProgress = $workItems->isNotEmpty()
            ? (int) round((float) $workItems->avg('progress_percent'))
            : ($legacyChecklistFallback
                ? (int) round((float) ($legacyChecklistItems->avg(fn ($item) => (bool) $item->is_done ? 100 : 0) ?? 0))
                : 0);
        $workSummary = [
            'total' => $legacyChecklistFallback ? $legacyChecklistItems->count() : $workItems->count(),
            'completed' => $legacyChecklistFallback ? $legacyChecklistItems->where('is_done', true)->count() : $workItems->where('status', 'completed')->count(),
            'in_progress' => $legacyChecklistFallback ? 0 : $workItems->where('status', 'in_progress')->count(),
            'blocked' => $legacyChecklistFallback ? 0 : $workItems->where('status', 'blocked')->count(),
        ...
        return view('synced.technical.maintenance.show', [
            'schedule' => $schedule,
            'siblings' => $siblings,
            ...
            'workItems' => $workItems,
            'legacyChecklistItems' => $legacyChecklistItems,
