...
if (!$phone) $phone = $cleanPhone($getVal($row, 'số_điện_thoại_phụ'));
if (!$phone) $phone = $cleanPhone($getVal($row, 'số_whatsapp'));
if (!$phone) $phone = $cleanPhone($getVal($row, 'phone'));
...
if ($createdTime) {
    $note = $note
        ...
        : 'Created: ' . $createdTime;
            ...
            if ($phone) $existsQuery->where('phone', $phone);
            else $existsQuery->where('email', $email);
