...
class MarketingProgressController extends Controller
{
    ...
    private function generateFromPlan(int $planId): void
    {
        ...
        if (!$plan) return;
        if (!$this->safeTable('mkt_plan_tasks')) return;
        ...
        if (!empty($ins)) DB::table('mkt_plan_tasks')->insert($ins);
    }
    ...
    private function summarize($tasks): array
    {
        ...
        foreach ($tasks as $t) {
            ...
            if (!isset($countByStatus[$st])) $countByStatus[$st] = 0;
            $countByStatus[$st] += 1;
        }
    ...
    public function monthly(Request $request)
    {
        ...
        if ($channel === 'other') $channel = 'ads'; // default friendly
    ...
    public function updateStatus(Request $request, int $id)
    {
        ...
        if (!$task) return back()->withErrors(['status' => 'Task không tồn tại.']);
