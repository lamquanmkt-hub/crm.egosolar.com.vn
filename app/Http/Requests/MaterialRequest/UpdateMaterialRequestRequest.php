...
/**
 * FormRequest validate dữ liệu cập nhật phiếu yêu cầu vật tư.
 */
class UpdateMaterialRequestRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            | Vật tư trong kho
            |--------------------------------------------------------------------------
            | Chỉ chọn kho + vật tư + số lượng.
            | Không nhập giá vốn ở đây.
            | Giá vốn lấy tự động từ kho/catalog.
            */
            ...
            |--------------------------------------------------------------------------
            | Khi tạo/sửa đơn, kỹ thuật chỉ nhập tên + số lượng + đơn vị.
            | Giá vốn chỉ nhập ở bước kho duyệt/xuất.
            */
    ...
    public function messages(): array
    {
        return [
            ...
            'items.*.note.max' => 'Ghi chú vật tư trong kho không được quá :max ký tự.',
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            /*
            |--------------------------------------------------------------------------
            | Validate từng dòng vật tư trong kho
            |--------------------------------------------------------------------------
            */
            ...
            /*
            |--------------------------------------------------------------------------
            | Validate từng dòng vật tư phụ / ngoài kho
            |--------------------------------------------------------------------------
            */
            foreach ($extraItems as $index => $row) {
                ...
                if (trim((string) ($row['name'] ?? '')) === '') {
                    $validator->errors()->add(
                        "extra_items.$index.name",
                        'Vui lòng nhập tên vật tư phụ / ngoài kho.'
                    );
                }

                if ((float) ($row['qty'] ?? 0) <= 0) {
                    $validator->errors()->add(
                        "extra_items.$index.qty",
                        'Số lượng vật tư phụ / ngoài kho phải lớn hơn 0.'
                    );
                }
            }
        });
